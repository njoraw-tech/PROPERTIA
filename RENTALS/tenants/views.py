        ...
        for idx, row in enumerate(rows, start=2):  # start=2 because row 1 is headers
            try:
                ...
                if not first_name or not last_name or not phone_number:
                    errors.append(f'Row {idx}: Missing required fields (first_name, last_name, phone_number)')
                    continue
                
                # Get unit if provided
                unit = None
                unit_name = row_lower.get('unit')
                if unit_name:
                    try:
                        unit = Unit.objects.get(name__iexact=unit_name)
                    except Unit.DoesNotExist:
                        errors.append(f'Row {idx}: Unit "{unit_name}" not found')
                        continue
                
                # Parse boolean and decimal fields
